...
/*
 * HPN-SSH server options.  All map to standard generated macros (no
 * SSHCONF_CUSTOM); the parser cases stay hand-written in servconf.c like
 * every other option.  HPNMemoryLimit uses multistate_hpnmemorylimit
 * (defined in servconf.c).  Field names match the pre-10.4 struct so
 * consumers outside servconf.{c,h} are unchanged.  Grouped in one block to
 * keep the fork's servconf footprint to a single greppable place.
 */
#define SSHD_CONFIG_ENTRIES_HPN \
SSHCONF_INTFLAG(tcp_rcv_buf_poll, TcpRcvBufPoll, SSHCFG_ALL, 1, SSHCFG_COPY_MATCH) \
SSHCONF_INTFLAG(hpn_disabled, HPNDisabled, SSHCFG_ALL, 0, SSHCFG_COPY_MATCH) \
SSHCONF_INTFLAG(hpn_workers_die, HPNWorkersDie, SSHCFG_ALL, 1, SSHCFG_COPY_MATCH) \
SSHCONF_INTFLAG(hpn_use_bundle, HPNUseBundle, SSHCFG_ALL, 1, SSHCFG_COPY_MATCH) \
SSHCONF_INTFLAG(hpn_writer_pool, HPNWriterPool, SSHCFG_ALL, 1, SSHCFG_COPY_MATCH) \
SSHCONF_INTFLAG(none_enabled, NoneEnabled, SSHCFG_ALL, 0, SSHCFG_COPY_MATCH) \
SSHCONF_INTFLAG(nonemac_enabled, NoneMacEnabled, SSHCFG_ALL, 0, SSHCFG_COPY_MATCH) \
